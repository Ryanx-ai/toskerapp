...
Development TID application migration APPLIED to exactly six inventoried users, including retained A/B: six unique canonical seven-character IDs, zero collisions, no legacy values at last audit. Every other public-table field/row fingerprint unchanged. No schema/history/migration metadata or Clerk binding/credential mutation. All four non-A/B users have retained authored history, so all six accounts are PRESERVED; zero account deletions. No aliases/DB format trigger or new historical SQL migration. Old canonical bootstrap can still create legacy IDs until rollout; re-inventory before release and recheck after deployment. Do not silently reset any newly discovered user or rerun historical migrations.

Fresh service/privacy/Room identity/Settings/TID/bootstrap suites, all19 migration hashes/catalog, DB invariants, TypeScript, ESLint (one pre-existing warning), production build PASS. Combined nine-category/six-width Settings/peer accent/privacy/Quiet test PASS with originals restored. New A/B exact lowercase/trim TID discovery, negative partial/legacy lookup, Profile/Namecard copy, Support href, Account gate and five-viewport geometry PASS; desktop/mobile captures inspected. See [night evidence/remaining gate](docs/MS7-2-IMPLEMENTATION.md) and [TID record](docs/MS7-2-TID-TRANSITION.md).

**Next action:** finish normal fresh-user registration with human assistance at Clerk's Cloudflare challenge; do NOT bypass it/change auth settings. Attempt `tosker.ms72.20260916+clerk_test@example.com` reached challenge, created **zero** Clerk/database users (readonly confirmed). Synthetic server bootstrap passes, but is NOT browser registration acceptance. Then fresh Profile/Namecard/discovery acceptance and exact coordinated cleanup if a user is created; remaining combined Room/Subroom/history/attention/failure/keyboard checks, exact MS7.2 Room cleanup, normal push/Git-backed canonical deploy/live A/B. No release from partial evidence. A/B browser profiles and existing local dev server preserved; no A/B credential changes. QA Rooms `ms722-frame-qa`/`ms722-second-qa` remain for unfinished gate (four conversations, zero messages/Hall). Preserve unrelated handoff/inheritance/Design Hub/Web/Art/research/experiments WIP and untouched/untracked `toskerArt/`. STOP for the night.

## MS7.2 — local checkpoints / founder decisions pending (2026-09-15)

Preserved latest validated Settings checkpoint `66ab5aa` and Room checkpoint `e11c37e`; find newer TID-foundation/docs checkpoint in Git. All independent slices remain LOCAL, not pushed/deployed or founder-ready/locked. New-user cryptographic7 TID generation/existing five-retry bootstrap boundary validated (forced collision/exhaustion and idempotent user/Profile/Sandbox; provider-name change cannot overwrite profile). Synthetic test accounts cleaned up; six real users still have their original legacy IDs. No TID migration/lookup/copy transition yet. [TID dependency/compatibility gate](docs/MS7-2-TID-TRANSITION.md), [implementation ledger](docs/MS7-2-IMPLEMENTATION.md), [walkthrough preparation](docs/MS7-2-FOUNDER-WALKTHROUGH.md).

RESUME after founder decisions: (1) keep Account provider panel gated or explicitly disable unsafe Clerk self-service deletion before integrating it; (2) supply monitored Support destination; (3) confirm old TIDs test-only or retain legacy aliases during seven-character transition. Do not change provider configuration or existing identifiers by assumption. Then implement the approved bounded paths, integrated service/browser/security/failure/desktop-mobile gate, exact remaining QA Room cleanup, normal push/Git-backed deploy/live A/B verification. No partial release. Development migrations0016–0018 already applied; all19 hashes/catalog pass; do not edit/rerun historical migration bytes. MS7.1 remains locked; MS7.3+ not started. Font rights remain unverified; current stack retained, no Art/Web/provider work. Preserve unrelated handoff/inheritance/Design Hub/Web/Art/research/experiments WIP and untouched/untracked `toskerArt/`.

## MS7.2.5 — local Settings/Brand/banner slice (2026-09-15)

Preserved Room checkpoint `e11c37e`; find newer Settings checkpoint in Git. Development0018_ms72_banner_preferences APPLIED, SHA256 `4cd763114922765b4d9dd2d04c1e0aff7790ce27f7bb87ef0d21ffd15c14361b`; private enum/column default all, old writers compatible. All19 hashes/catalog PASS (25tables/161columns/78constraints/12enums). Do not edit or manually rerun0018. Nine addressable Settings categories, five privacy-scoped accents, owner banner preferences and metadata reconciliation implemented. Service/privacy/mode matrix, six-width category/dirty/peer accent, same-user hidden-tab refresh and actual B→A banner/unread tests PASS; type/lint/build/diff PASS. Three exact banner QA messages/notifications removed, scoped Rooms now empty; preferences restored. Full provider Account panel remains GATED: readonly Clerk A/B deleteSelfEnabled true; founder decision requested, no provider/auth/credential change. Support destination missing. Next: safe seven-character new-TID foundation/existing-ID transition decision, full integrated gate/docs/cleanup; no partial push/deploy. Existing legacy TIDs remain unchanged; founder asked whether publicly displayed legacy IDs have been externally shared. See [ledger](docs/MS7-2-IMPLEMENTATION.md). Preserve unrelated WIP and untouched/untracked `toskerArt/`; no MS7.3+.

## MS7.2.4 — local Room identity slice (2026-09-15)

Preserved Profile checkpoint `882c457`; find newer Room checkpoint in Git. Development migration `0017_ms72_room_identity` APPLIED, SHA256 `67b1458fd3d85507906de70b12f132307633deb5648d8a1ef6fe3ac1a9af5267`: nullable membership nickname, random revision UUID and minimal owner-reset accountability table (no old nickname text). All18 hashes/catalog PASS,25tables/160columns/78constraints/11enums. Do not edit or manually rerun applied0017. Actor-only nickname/editor and owner reset, contextual history/Hall/member/notification names, parent-child inheritance and scoped Namecards implemented. Rollback isolation/history/metadata tests and real A/B save/reset/stale-draft/child-mention/six-width browser checks PASS; long-name People wrap fixed and visually rechecked. Original QA nickname restored; no test messages. Type/lint/build/diff pass. See [execution ledger](docs/MS7-2-IMPLEMENTATION.md) for evidence/limits. Next: Account/Brand/banner preferences, safe TID transition, integrated release gate and exact QA cleanup. No push/deploy, MS7.3+, provider/font/Art work. Preserve unrelated WIP and untouched/untracked `toskerArt/`.
...
[MS7.2 scope, acceptance and policy gates](docs/MS7-2-PROFILE-SETTINGS.md). Own Profile now edits global display name and existing manual status through actor-only partial updates; identifiers remain read-only, other viewers' private aliases stay private. Account Settings links to real Profile/Support and labels unsupported categories without dead controls. No schema/provider/transport/history/media changes. Service isolation/invalid-forged inputs/metadata-only writes passed with synthetic fixtures rolled back; browser save/reload/peer projection and mobile/desktop modal/focus checks passed. Workspace gutter CSS was incorrectly narrowing the new native dialog; modal exclusion fixed/retested. No FP4 fixture recreation. Local Clerk/session refresh caveats persist; normal QA sign-in recovered, no auth bypass or environment change.

Next: review the bounded checkpoint and decide parent-Room nickname ownership/moderation, public identity/privacy/identifier policy and Personal Brand governance before implementing those fields. Do not infer answers, deploy this partial MS7.2 slice, reopen locked Chat/Room work, or begin MS7.3+. Preserve unrelated mixed handoff/inheritance/Design Hub/Web/Art/research/experiments WIP and untouched/untracked `toskerArt/`.

## MS7.1-FP4 — LOCKED (2026-09-14)

MS7.1 locked under founder's explicit conditional FP4 authorization. Live-tested application `f3ab7d017ce92bee34a473fbb22230b26cffdaa4`, exact Git deployment `dpl_HXkQgqRzL7h7zDk4Bn9F4yRc6qpX` READY at canonical `https://toskerapp.vercel.app/` HTTPS200. [FP4 acceptance/control/cleanup record](docs/MS7-1-FP4-FOUNDER-PATCH.md), [identity contract](docs/IDENTITY-NAMECARD-CONTRACT.md). Namecard, private aliases/Common Rooms, six-section scoped Settings, Hall reference interactions/source author/edit/navigation/Nuke and private sidebar pins/order pass local and live A/B acceptance. All16 migrations/catalog/invariants, build/type/lint/secret checks, six widths, keyboard/reduced motion and21-group retained history pass. Exact local/live fixtures removed;29 messages/9 notes/2 pins and real users/canonical Personal/Sandboxes preserved. Do not rerun deleted fixture routes or seed/cleanup suites. DK Longreach remains deferred/untouched; Development infrastructure/deferred controls remain truthful debt, not Production certification. **Next authorized work: bounded MS7.2 Profile + Settings design-first slice from this lock; do not reopen Chat/Room/transport/history or start MS7.3+.** Preserve unrelated mixed handoff/inheritance/Design Hub/Web/Art/research/experiments WIP and untracked `toskerArt/`. Newer documentation-only HEAD may carry identical application source; verify Git/deployment. Local Clerk/session harness caveats remain documented; no credential changes or auth bypass.

## MS7.1-FP3 — live / founder review required (2026-09-13)

FP3 IMPLEMENTED, PUSHED AND LIVE-VERIFIED. Product `049c8234c5043930b8d279fdc9d4bdcdedd72023`; exact Vercel `dpl_7YC7TH2FP8AcGDZopsFkjhQPSohe` READY at `https://toskerapp.vercel.app/` HTTP200. A `389a7a6`, B/C `8b97bd9`, migration integrity `6da3c75` and Room-history `15a1dc4` preserved. [FP3 implementation/security/Settings/scheduler/validation/cleanup/debt](docs/MS7-1-FP3-FOUNDER-PATCH.md). Local six-width/keyboard/privacy/attention, typecheck/lint/build/secret scan, all15 hashes/current catalog/invariants PASS; live normal A/B Nuke/pin/reply/draft/reload/privacy/Settings/mute/nickname/retained-history PASS. Documentation-only closeout may carry newer HEAD/deployment of identical application source; inspect Git/alias. No provider/schema/migration/font changes; `toskerArt/` untouched/untracked and unrelated WIP preserved. **STOP FOR FOUNDER WALKTHROUGH; MS7.1 NOT LOCKED; MS7.2 NOT STARTED.**

Exact QA cleanup complete:1 Room/1 child/83 contained messages plus6 private fixture messages/2 private notes/comments and16 related notifications; real users, Personal/Sandbox conversations and unrelated Hall ordering preserved. Post-cleanup6 users/profiles/Sandboxes,2 Rooms,5 Personal conversations,29 messages,8 notes/1 pin; duplicate/orphan checks0, all15 hashes/catalog PASS. No application undo. Do not rerun/recreate removed fixture routes or one-shot suites. Owned fp3-a/b/check browsers and local server closed; profiles preserved. Local A Clerk session-refresh loop needed normal fresh-profile recovery (no auth bypass/config change or proven cause); canonical normal A/B smoke passed. Scheduling is only a Development disclosure (MS7.6), Personal Brand static only, existing font stack unchanged. Next action: founder walkthrough, no background development or inferred milestone lock.
...
Direct recipient-consent invites from accepted Friends/exact username; owner-only single encrypted recoverable share with24h default/1h/7d, same Room-lock join/revoke/replace; pending zero data/realtime access. Owner-only durable Subroom order with lazy deterministic legacy normalization/stale conflict guard; authorized relative subsets. Derived ordinary-message burst groups preserve exact events/bell/attention. Development-only deferred Call/Video/Files/Translate disclosures, no media/storage/translation services. Migration0014 applied(15 migrations/48 FKs); dedicated invitation-encryption secret exists local+Vercel Development/Production-review, never Preview/client. Do not print/overwrite; separation/rotation remains MS7.6 debt. Chat/history/retry/Ably unchanged.

Fresh build/TypeScript/lint/schema/invariants, invite races, Room lifecycle/Ably revocation, order, grouping/attention and secret scan pass. Initial unchanged Slice A full service acceptance inherited. Local A/B full contract and25-event burst pass; touched UI at320/390/430/768/1440 plus keyboard/nested Escape/refocus and Personal/Subroom/Hall/Sandbox checks pass. Small live A/B direct invite→accept→message; share→leave/rejoin; shared order; two-message group and separate selected mention pass. Browser errors empty/deployment error sample0. Harness interruptions/stale selectors and local slow read settling are documented; no physical-device/screen-reader or instantaneous-latency claims.

Exact cleanup complete: local2 Rooms/6 children/5 conversations/28 messages/35 notifications/4 non-login QA users; live1 Room/2 children/3 conversations/4 messages/5 notifications. No application undo. Post-cleanup6 real users/profiles/Sandboxes,2 preserved Rooms,5 Personal conversations,28 preserved messages,8 notes/1 pin; duplicate/orphan checks0. One-shot scripts now refuse removed fixtures; do not rerun old routes/recreate large suites. Only owned QA browsers/server should be closed; preserve profiles and unrelated docs/Art/Web/research/experiments WIP. Founder alone approves lock. No background continuation.
...
Product checkpoint `80f504fe50dc993d4fc036d1a89cf718c883082c` is pushed to main; Vercel `dpl_CqdSshonsvai2YFgBzkgRyh98fwn` READY with canonical `https://toskerapp.vercel.app` assigned. Local TypeScript/lint/build/secret scan, focused attention/navigation, five widths and keyboard passed; fresh canonical isolated A/B send, stale-request and mobile + smoke PASS. Exact guarded FP1 cleanup removed its Room/two children/three conversations/four messages/invite/QA profile/request history, with owned counts zero. Browsers and owned local server closed. Do not rerun removed fixture routes or old giant suites. A documentation-only closeout may create a newer HEAD/deployment with identical application source; inspect Git/alias. Preserve unrelated mixed handoff/Design Hub/Web/Art/research/experiments WIP. **MS7.1 remains unlocked; STOP FOR FOUNDER REVIEW. No MS7.2.**
