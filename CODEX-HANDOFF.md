...
## MS7.1-FP4 — active implementation (2026-09-14)

Founder authorizes FP4 acceptance → canonical deploy/live smoke → MS7.1 lock only if green, then bounded MS7.2. Baseline `16c5e4d`; [FP4 ledger/control matrix](docs/MS7-1-FP4-FOUNDER-PATCH.md) and [identity contract](docs/IDENTITY-NAMECARD-CONTRACT.md). Namecard/scoped Settings, Hall reference interactions/shared source edit and private sidebar pins are implemented. Migration0015 applied to Development; all16 hashes/catalog and invariants pass, older migrations untouched. Local Namecard privacy/withdrawal, Hall lifecycle/source jump, private pin order/reload/separate sign-in, six-width identity/Settings, mobile Hall/collapsed sidebar, keyboard, 21-group retained history and small Personal/Room/child realtime/privacy/removal checks pass. Source highlight DOM-commit race fixed and rerun. Fresh build/type/lint/secret checks pass. Guarded local cleanup removed exact two QA Rooms/one child/65 messages/four Hall objects/two notifications; users, canonical Personal/Sandbox and unrelated Hall order preserved. Do not rerun deleted fixture routes/setup suites. No FP4 deploy or lock yet. Next: focused validated commit → normal push/main → exact Git-backed canonical READY/alias → small live A/B using `fp4-live-fixtures.ts`, exact cleanup → record lock if green → first safe MS7.2 slice. DK Longreach deferred, untouched. Preserve unrelated handoff/inheritance/Design Hub/Web/Art/research/experiments WIP and untracked `toskerArt/`. Recurring local stale Clerk session/automation timeouts are documented, not a proven key mismatch; normal fresh test sign-in only.
