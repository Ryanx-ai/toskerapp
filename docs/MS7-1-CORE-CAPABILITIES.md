...
The [FP4 control matrix and evidence](MS7-1-FP4-FOUNDER-PATCH.md) distinguish implementation from fresh/inherited acceptance. Canonical application`f3ab7d0` READY/live A/B smoke and exact cleanup pass; founder's conditional lock criteria are met. [Identity contract](IDENTITY-NAMECARD-CONTRACT.md) is the MS7.2 bridge. Existing Development debt disclosures remain explicitly deferred, not shipped media/translation/scheduling. Font rights are not established; no supplied font is shipped.
