...
async function open(s,path){await run(s,"open",`http://localhost:3000${path}`);await until(s,"!!document.querySelector('.conversation-surface')&&!document.querySelector('.chat-load-state')","Chat loaded");}
...
await run(a,"open","http://localhost:3000/app");await until(a,"!!document.querySelector('[aria-label=\"Start a chat or create a Room\"]')","app ready");
...
await click(a,"Invite");await until(a,"document.querySelector('.invite-link input')?.value.startsWith('http://localhost:3000/join/')","invite generated");
