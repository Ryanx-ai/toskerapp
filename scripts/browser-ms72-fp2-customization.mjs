...
async function open(s,section){await run(s,'open',origin+'/settings?section='+section);await until(s,`document.querySelector('.scoped-settings-body')?.getAttribute('aria-label')===${JSON.stringify({brand:'Personal Brand',appearance:'Appearance',privacy:'Privacy',account:'Account'}[section])}`,'Settings '+section);}
async function save(){if(await ev(a,"!document.querySelector('.scoped-settings-footer .primary-action').disabled")){await button(a,'Save changes');await until(a,"document.querySelector('.owner-profile-save [role=status]')?.textContent==='Changes saved.'&&document.querySelector('.scoped-settings-footer .primary-action').disabled",'save',45000);}}
...
async function choose(values){for(const [key,value] of Object.entries(values))await run(a,'check',`input[name=${key}][value=${value}]`);}
