/** Exact FP4 browser-created Room only; no retained Room or user cleanup. */
...
async function main(){
 ...
 if(mode==='cleanup-receipts'){
  // Supply only exact receipt IDs recorded by this FP4 browser run, after UI Nuke.
  ...
  await db.transaction(async tx=>{
   ...
   assert(found.every(m=>m.authorId===a&&m.conversationId==='be192eac-38c6-4d46-a6d2-bea19fa324fa'&&m.body===''&&m.deletedAt&&m.createdAt>=new Date('2026-09-26T00:00:00+08:00')));
   ...
   console.log('Removed exact already-nuked FP4 Personal receipts only; retained Personal history untouched; no application undo.',ids);
  });return;
 }
 assert(['attach','inspect','cleanup','cleanup-empty'].includes(mode));assert(/^fp4-review-[a-f0-9]{6}$/.test(slug));
 await db.transaction(async tx=>{
  const [room]=await tx.select().from(rooms).where(eq(rooms.slug,slug)).for('update');assert(room&&room.ownerId===a&&room.name==='FP4 Review');
  ...
  if(mode==='attach'){
   ...
   const [child]=await tx.insert(subrooms).values({roomId:room.id,name:'FP4 Side Trip',createdBy:a,visibility:'everyone'}).returning();
  ...
  if(mode==='cleanup'||mode==='cleanup-empty'){
   assert(room.createdAt>=new Date('2026-09-26T00:00:00+08:00'));
   ...
   else {assert.equal(children.length,1);assert(children.every(c=>c.createdBy===a&&c.name==='FP4 Side Trip'));assert.equal(chats.length,2);}
   ...
   console.log('Removed only the verified FP4 synthetic Room, child, memberships and empty nuked receipts. No application undo; all retained users/Rooms/Personal history preserved.');
  }
 });
}
