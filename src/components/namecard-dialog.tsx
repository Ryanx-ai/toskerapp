...
function NamecardDialog({ userId, onClose }: { userId: string; onClose: () => void }) {
  ...
  return <ModalLayer onClose={() => { if (!busy) onClose(); }}><section className="identity-dialog contextual-namecard" aria-labelledby={`${id}-title`}>
    ...
    {nickname ? <button className="inline-back" disabled={busy} onClick={() => setNickname(false)}><ArrowLeft size={15} />Back</button> : <p className="eyebrow">Namecard{person?.self ? " · You" : ""}</p>}
    {person ? <>
      ...
      {nickname && person.connectionId ? <PrivateNicknameForm target={{ id: person.connectionId, name: person.displayName, nickname: person.nickname }} onBusy={setBusy} onSaved={refresh} /> : <>
        {!person.self ? <div className="namecard-actions"><button className="primary-action" disabled={busy} onClick={async () => {
          ...
          {person.connectionId ? <button className="quiet-action" disabled={busy} onClick={() => setNickname(true)}>Private nickname</button> : null}</div> : <Link className="quiet-action" href="/profile" onClick={onClose}>Your Profile</Link>}
