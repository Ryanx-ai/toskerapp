...
export function OwnProfileEditor({ identity, onClose }: { identity: CanonicalIdentity; onClose: () => void }) {
  ...
  return <SettingsShell title="Edit Profile" context="Your global identity" onClose={onClose} busy={busy} sections={[
  ...
  ]} footer={<div className="owner-profile-save">{error ? <p role="alert">{error}</p> : null}<div className="overlay-actions"><button disabled={busy} onClick={onClose}>Cancel</button><button className="primary-action" disabled={busy || !changed || !name.trim()} onClick={async () => {
