...
export function SubroomOrderRow({ roomId, ids, id, name, owner, children, actions }: { roomId: string; ids: string[]; id: string; name: string; owner: boolean; children: ReactNode; actions?: (close: () => void) => ReactNode }) {
  ...
  if (!owner && !actions) return children;
  const index = ids.indexOf(id);
  return <div className={`fp2-order-row ${over ? "drag-target" : ""}`} data-subroom-id={id}
    onDragOver={(event) => { if (owner && !busy && event.dataTransfer.types.includes(mime)) { event.preventDefault(); event.dataTransfer.dropEffect = "move"; setOver(true); } }}
    ...
    {children}
    <button className="fp2-order-handle" aria-label={`${actions ? "Actions for" : "Reorder"} ${name}`} aria-haspopup="dialog" aria-expanded={open} disabled={busy} draggable={owner && !busy}
      ...
      onDragEnd={() => setOver(false)}>{actions ? <MoreHorizontal size={16} aria-hidden="true" /> : <GripVertical size={16} aria-hidden="true" />}</button>
    {open ? <InteractionPopover anchor={anchor} onClose={() => setOpen(false)} label={`${actions ? "Actions for" : "Reorder"} ${name}`}><div className="message-action-list">
      {actions?.(() => setOpen(false))}
      {owner ? <><button disabled={index <= 0 || busy} onClick={() => void move(id, ids[index - 1])}>Move earlier</button>
      <button disabled={index < 0 || index >= ids.length - 1 || busy} onClick={() => void move(id, ids[index + 1])}>Move later</button><p className="settings-scope">Subroom order is shared.</p></> : null}
