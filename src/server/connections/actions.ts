...
export async function listConnectionsAction() {
  const actor = await requireCurrentActor();
  return readConnections(getDatabase(), actor.userId);
}
