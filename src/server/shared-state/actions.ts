...
export async function listHallItemsAction(conversationId: string, archived = false) {
  ...
  const rows = await db
    ...
    .where(and(archived ? isNotNull(hallItems.archivedAt) : isNull(hallItems.archivedAt), scope, sql`(${hallItems.sourceMessageId} is null or (${messages.id} is not null and ${messages.deletedAt} is null))`))
    .orderBy(asc(hallItems.position), asc(hallItems.createdAt), asc(hallItems.id));
  return rows.map((item) => ({ ...item, canModerate: owner || item.authorId === actor.userId, sourceDeletedAt: undefined, body: item.body ?? item.sourceBody ?? "", createdAt: item.createdAt.toISOString(), archivedAt: item.archivedAt?.toISOString() ?? null }));
}
